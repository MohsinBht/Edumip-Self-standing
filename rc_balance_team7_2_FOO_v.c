/**
* @example rc_balance_team7_2_FOO_v
*
...
#include "rc_balance_team7_2_FOO_v_defs.h"
#include <rc/servo.h>
 ...
 */
FILE *file;
double drive_stick, turn_stick,servo_stick;
int speech_stick;
...
typedef struct core_state_t{
    ...
    double theta_est;
    double theta_dot_est;
	double phi_dot_vit;
    double phi_est;
	double phi_dot_est;
	double zeta_p_theta;
    double zeta_p_theta_dot;
    double zeta_p_phi;
    double zeta_p_phi_dot;
...
m_input_mode_t m_input_mode = SPEAK;
...
void print_usage(){
	printf("\n");
	printf("-i {speak|joy|stdin|none}     specify input\n");
...
int main()
{
	...
	printf("\nPress and release MODE button to toggle mode\n");
	...
	if(rc_filter_alloc_from_arrays(&D1, DT, D1_num, D1_NUM_LEN, D1_den, D1_DEN_LEN)){
		fprintf(stderr,"ERROR in rc_balance_team7_2_FOO_v, failed to make filter D1\n");
		return -1;
	}
	D1.gain = D1_GAIN;
	...
	if(rc_filter_alloc_from_arrays(&D2, DT, D2_num, D2_NUM_LEN, D2_den, D2_DEN_LEN)){
		fprintf(stderr,"ERROR in rc_balance_team7_2_FOO_v, failed to make filter D2\n");
		return -1;
	}
	D2.gain = D2_GAIN;
	...
	if(rc_filter_pid(&D3, D3_KP, D3_KI, D3_KD, 4*DT, DT)){
		fprintf(stderr,"ERROR in rc_balance_team7_2_FOO_v, failed to make steering controller\n");
		return -1;
	}
	...
	while(cstate.vBatt<1.0 && rc_get_state()!=EXITING) rc_usleep(1000);
	...
	while(rc_get_state()!=EXITING){
		...
		case SPEAK:
			...
			case 0 :
				...
				continue;
			case 1 :
				setpoint.gamma_dot = SPEAK_TURN;
				continue;
			case 2 :
				setpoint.gamma_dot = -1 * SPEAK_TURN;
				continue;
			case 3 :
				setpoint.phi_dot   = SPEAK_UP;
				continue;
			}
		case STDIN:
			i = 0;
...
void balance_controller()
{
	...
	******************************************************************/
	...
	//double Loo[] = LOO;
	//double Action_mesures[] = ACTION_MESURES;
    double Lfoo_theta[] = LFOO_THETA;
    double Lfoo_theta_dot[] = LFOO_THETA_DOT;
    double Lfoo_phi[] = LFOO_PHI;
    double Lfoo_phi_dot[] = LFOO_PHI_DOT;
    double Bfoo[] = BFOO;
    double Afoo_theta[] = AFOO_THETA;
    double Afoo_theta_dot[] = AFOO_THETA_DOT;
    double Afoo_phi[] = AFOO_PHI;
    double Afoo_phi_dot[] = AFOO_PHI_DOT;

	theta_a =  - atan2(mpu_data.accel[2],mpu_data.accel[1]) + BOARD_MOUNT_ANGLE;
	cstate.theta = ((1-K )*(cstate.theta + (DT * ((M_PI/180.0)*mpu_data.gyro[0]))))+(K*theta_a);
	...
	cstate.gamma = (cstate.wheelAngleR-cstate.wheelAngleL)* (WHEEL_RADIUS_M/TRACK_WIDTH_M);
	cstate.theta_dot = mpu_data.gyro[0]*M_PI/180;
	
	// Full order observer estimated states
	cstate.theta_est = cstate.zeta_p_theta;
    cstate.theta_dot_est = cstate.zeta_p_theta_dot;
    cstate.phi_est = cstate.zeta_p_phi;
    cstate.phi_dot_est = cstate.zeta_p_phi_dot;
	...
	*************************************************************/
	double K_s[]=KS;
	cstate.d1_u= (N_s*setpoint.phi_dot-(K_s[0]*cstate.theta +K_s[1]*cstate.theta_dot+K_s[2]*cstate.phi_dot_est))/2;;
	...
	***********************************************************/
	...
	// Calculated previous state of the full order observer 
    cstate.zeta_p_theta = Afoo_theta[0]*cstate.theta_est + Afoo_theta[1]*cstate.theta_dot_est + Afoo_theta[2]*cstate.phi_est + Afoo_theta[3]*cstate.phi_dot_est+ Lfoo_theta[0]*(cstate.theta - cstate.theta_est) + Lfoo_theta[1]*(cstate.theta_dot - cstate.theta_dot_est) + Lfoo_theta[2]*(cstate.phi-cstate.phi_est) + Bfoo[0]*(dutyL + dutyR);
	cstate.zeta_p_theta_dot = Afoo_theta_dot[0]*cstate.theta_est + Afoo_theta_dot[1]*cstate.theta_dot_est + Afoo_theta_dot[2]*cstate.phi_est + Afoo_theta_dot[3]*cstate.phi_dot_est+ Lfoo_theta_dot[0]*(cstate.theta - cstate.theta_est) + Lfoo_theta_dot[1]*(cstate.theta_dot - cstate.theta_dot_est) + Lfoo_theta_dot[2]*(cstate.phi-cstate.phi_est) + Bfoo[1]*(dutyL + dutyR);
    cstate.zeta_p_phi = Afoo_phi[0]*cstate.theta_est + Afoo_phi[1]*cstate.theta_dot_est + Afoo_phi[2]*cstate.phi_est + Afoo_phi[3]*cstate.phi_dot_est+ Lfoo_phi[0]*(cstate.theta - cstate.theta_est) + Lfoo_phi[1]*(cstate.theta_dot - cstate.theta_dot_est) + Lfoo_phi[2]*(cstate.phi-cstate.phi_est) + Bfoo[2]*(dutyL + dutyR);
    cstate.zeta_p_phi_dot = Afoo_phi_dot[0]*cstate.theta_est + Afoo_phi_dot[1]*cstate.theta_dot_est + Afoo_phi_dot[2]*cstate.phi_est + Afoo_phi_dot[3]*cstate.phi_dot_est+ Lfoo_phi_dot[0]*(cstate.theta - cstate.theta_est) + Lfoo_phi_dot[1]*(cstate.theta_dot - cstate.theta_dot_est) + Lfoo_phi_dot[2]*(cstate.phi-cstate.phi_est) + Bfoo[3]*(dutyL + dutyR);
    return;
}
...
void* printf_loop(__attribute__ ((unused)) void* ptr){
	...
	while(rc_get_state()!=EXITING){
		...
		file = fopen("/home/debian/bin/data.txt", "r");
		fscanf(file, "%lf", &turn_stick);
		fscanf(file, "%lf", &drive_stick);
		fscanf(file, "%lf", &servo_stick);
		fscanf(file, "%d", &speech_stick);
		rc_servo_send_pulse_normalized(1,servo_stick);
		//fscanf(file,"%s", &num); //%lf
		//fgets(num, 20, file);
		fclose(file);	
		...
		if(new_rc_state == RUNNING){
			...
			printf("%7.3f  |", cstate.phi_dot_vit);
			printf("%7.3f  |", cstate.d1_u);
			printf("%7.3f  |", cstate.d3_u);
			printf("%7.3f  |", cstate.vBatt);
			printf("%7.3f  |", turn_stick);
			printf("%7.3f  |", drive_stick);
			printf("%7.3f  |", servo_stick);
 ...
 */
 // change between joystick mode and speak mode on mode button release 
void on_mode_release()
{
	
	if(m_input_mode == JOY){
		m_input_mode = SPEAK;
		printf("using m_input_mode = SPEAK\n");
	}
	else {
		m_input_mode = JOY;
		printf("using m_input_mode = JOY\n");
	}
